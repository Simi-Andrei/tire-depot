  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        setSelectedTires(estimate.tires || []);
        const tireIds = estimate.tires.map((tire) => tire._id);
        form.setValue("correspEstimate", estimate._id);
        form.setValue("customerFirstname", estimate.customerFirstname || "");
        form.setValue("customerLastname", estimate.customerLastname || "");
        form.setValue("periodInMonths", estimate.periodInMonths || "");
        form.setValue("tires", tireIds || []);
        form.setValue("rims", estimate.rims || false);
        form.setValue("pricePerMonth", estimate.pricePerMonth || 0);
        setCurrentEstimate(estimate);

        const tiresRes = await fetch("/api/tires");
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="my-1">
        <Card>
          ...
          {loadingData ? (
            <CardContent className="min-h-96 grid place-items-center">
                ...
                <div className="col-span-2">
                      ...
                      <FormItem className="flex flex-col">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                                    ...
                                    {tires.map((tire, index) => (
                                      <CommandItem
                                        ...
                                        onSelect={(currentValue) => {
                                          ...
                                          const updatedSelectedTires = [
                                            ...selectedTires,
                                            tires.find(
                                              (tire) => tire._id === newValue
                                            ),
                                          ];
                                          setSelectedTires(
                                            updatedSelectedTires
                                          );
                                          setTiresOpen(false);
                                        }}
                                      >
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (tiresValue) {
                                const tireToDuplicate = tires.find(
                                  (tire) => tire._id === tiresValue
                                );
                                if (tireToDuplicate) {
                                  const updatedSelectedTires = [
                                  ...
                                  const updatedTireIds = [
                                    ...form.getValues("tires"),
                                    tireToDuplicate._id,
                                  ];
                                  form.setValue("tires", updatedTireIds);

                                  // Set tiresValue to the last duplicated tire's ID
                                  setTiresValue(tireToDuplicate._id);
                                }
                              }
                            }}
                            className="h-10"
                  ...
                  {selectedTires.length !== 0 && (
                    <div>
                      ...
                      {selectedTires.map((selectedTire, index) => {
                        return (
                          <div
                          ...
                          >
                            ...
                            <Button
                              ...
                              onClick={() => {
                                const updatedSelectedTires =
                                  selectedTires.filter((_, i) => i !== index);
                                setSelectedTires(updatedSelectedTires);
                                form.setValue(
                                  "tires",
                                  updatedSelectedTires.map((tire) => tire._id)
                                );
                              }}
                            >
